...
下载 [Releases](https://github.com/Leo-zzl/sites-aoi-analysis/releases) 中的安装包，双击运行即可。**无需安装 Python 环境**。

![主界面截图](docs/assets/screenshot-main.png)

#### 操作步骤

1. **上传 AOI 数据**
   - 点击「AOI 数据」卡片中的「选择文件」按钮
   - 选择包含 AOI 边界和场景名称的 Excel 文件（如 `AOI样例数据.xlsx`）
   - 工具会自动识别「场景字段」和「边界字段」，请确认下拉框中的映射是否正确

2. **上传站点数据**
   - 点击「站点数据」卡片中的「选择文件」按钮
   - 选择包含基站信息的 Excel 文件（如 `基站站点样例数据.xlsx`）
   - 工具会自动识别以下字段映射，请逐一确认：
     - **名称**、**经度**、**纬度**、**频段**、**覆盖类型**

3. **校验数据**
   - 确认所有字段映射无误后，点击「校验数据」按钮
   - 校验通过后，状态提示变为绿色「校验通过，可以点击【开始分析】」
   - 若校验失败，请根据提示检查字段映射或数据格式

4. **选择输出路径**
   - 在「输出文件」卡片中，点击「浏览」按钮选择结果保存位置
   - 默认文件名格式：`小区_AOI匹配_1000米限制_YYYYMMDD_HHMMSS.xlsx`

5. **开始分析**
   - 点击「开始分析」按钮，进度条实时显示当前处理阶段
   - 分析过程中可随时点击「停止」按钮中断任务
   - 分析完成后，结果自动保存至所选路径，界面显示统计摘要
