"""Generate large-scale stress test data."""
...
AOI_OUTPUT = "tests/fixtures/sample_data/stress_aoi_data.xlsx"
SITE_OUTPUT = "tests/fixtures/sample_data/stress_site_data.xlsx"
...
def generate_stress_aoi_data(n=1000):
    records = []
    for i in range(n):
    ...
    print(f"Generated stress AOI data: {AOI_OUTPUT} ({len(df)} rows)")
    return df


def generate_stress_site_data(n=100_000):
    records = []
    for i in range(n):
        lon = random.uniform(LON_MIN, LON_MAX)
        lat = random.uniform(LAT_MIN, LAT_MAX)
        records.append({
            "小区名称": f"CELL_{i:06d}_{uuid.uuid4().hex[:6]}",
    ...
    print(f"Generated stress Site data: {SITE_OUTPUT} ({len(df):,} rows)")
    return df


if __name__ == "__main__":
    random.seed(42)
    generate_stress_aoi_data(1000)
    generate_stress_site_data(100_000)
